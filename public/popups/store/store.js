(function () {
  ...
  function renderGames() {
    ...
    btnName.addEventListener('click', function () { handleSort('gameName'); });
    ...
    btnPrice.addEventListener('click', function () { handleSort('price'); });
    ...
    getSortedGames().forEach(function (game) {
      ...
      const priceText = game.price === 0 ? 'Free' : '$' + game.price.toLocaleString('en-US');
      ...
      btn.textContent = 'Buy';
      btn.addEventListener('click', function () { confirmBuy(game); });
  ...
  function showConfirm(title, message, onConfirm) {
    ...
    `;
    el.querySelector('.confirm-cancel').addEventListener('click', function () { el.remove(); });
  ...
  function showAlert(title, message) {
    ...
    `;
    el.querySelector('.confirm-ok').addEventListener('click', function () { el.remove(); });
    document.body.appendChild(el);
  }
  ...
  function confirmBuy(game) {
    ...
    showConfirm(
      'Confirm Purchase',
      'Buy <strong>' + game.gameName + '</strong> for <strong>' + priceText + '</strong>?',
      function () { executeBuy(game); }
    );
  }
  ...
  async function executeBuy(game) {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 400) {
          showAlert('Insufficient Funds', 'You don\'t have enough balance to buy <strong>' + game.gameName + '</strong>.');
        }
        return;
      }
      window.dispatchEvent(new CustomEvent('balancechange', { detail: { balance: data.balance } }));
      window.dispatchEvent(new CustomEvent('gamepurchased', { detail: { game: game } }));
      allGames = allGames.filter(function (g) { return g._id !== game._id; });
      renderGames();
    } catch {
