...
const checkUsername = debounce(async (value) => {
  ...
  if (value.length < 3) {
    setHint(usernameHint, "Must be at least 3 characters", "error");
    return;
  }
  setHint(usernameHint, "Checking…", "checking");
  try {
    const res = await fetch(
      `/api/auth/check/username?value=${encodeURIComponent(value)}`,
    );
...
const checkEmail = debounce(async (value) => {
  ...
  if (!EMAIL_REGEX.test(value)) {
    setHint(emailHint, "Enter a valid email (x@x.x)", "error");
    return;
  }
  setHint(emailHint, "Checking…", "checking");
  try {
    const res = await fetch(
      `/api/auth/check/email?value=${encodeURIComponent(value)}`,
    );
    const data = await res.json();
    data.available
